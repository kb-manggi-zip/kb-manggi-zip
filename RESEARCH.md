...
>
> ### ✅ 추가 반영 (2026-07-26) — 출처: `ref/잔여리서치_확정_0726.md`
> - **A2 전월세전환율 0.055 → `0.0475`**: 2026.7.16 한은 기준금리 인상(2.50→2.75%) → 법정 "기준금리+2.0%p" = 4.75%. 렌트홈 계산기 확인. `renewal.yaml`+`engine/rules.ts` 반영·fixtures 재생성. **P3(월세) 비교표 직접 영향** (환산액↑).
> - **생애최초 취득세 감면 = 적용 확정**: 2028.12.31까지 연장(지특법 §36의3), 12억↓ 200만 한도. `lending_regulated.yaml` 주석 🔴→✅. (감면 자체는 이미 compare에 -200만 적용 중이었음 → 이제 근거 확정)
> - **구파일 정리**: 미사용 `lending.yaml`·`guarantee.yaml` 삭제 → 시작 시 미검증 경고 12→5줄(남은 5 = 진짜 미검증 활성값).
