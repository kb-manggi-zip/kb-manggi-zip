...
2. **AI가 상황을 이해합니다** — "재택근무예요"라고 한 줄 적으면,
   AI가 "통근보다 동네 환경을 더 볼까요?"라고 해석해 제안하고,
   본인이 맞다고 확인해야만 반영됩니다.
...
그래서 **상황을 이해하는 일은 AI가** 합니다. 자유롭게 적은 한 줄을
해석하고, 입력끼리 모순되면 되묻습니다.
...
동네를 고르는 가중치도 마찬가지입니다. "통근을 제일 크게 본다(46%)"는
저희 생각이 아니라, 국토부 주거실태조사에서 실제 이사 사유 1·2위가
직주근접(30.6%)과 교통(25.5%)이기 때문입니다. 본인이 "재택"이라고
말하면 그 사람에 한해 통근 비중을 낮춥니다 — 통계는 평균의 기본값이고,
본인의 말은 그 사람의 사실이니까요.
...
같은 동네라도 카페를 좋아하는 1인 가구와 아이 있는 신혼부부는
다른 하루를 받습니다.
...
| 문서 | 내용 |
|---|---|
| [docs/구현_아키텍처_상세.md](docs/구현_아키텍처_상세.md) | 단계별 컨텍스트 구성, 스키마, 계산 계보 전체 (실제 실행값) |
| [docs/서비스_E2E_아키텍처.md](docs/서비스_E2E_아키텍처.md) | 전체 설계와 데이터 흐름 |
| [docs/검증_손계산대조.md](docs/검증_손계산대조.md) | 3개 시나리오 수기 계산 대조 결과 |
...
| 데이터 | 출처 |
|---|---|
| 실거래 64,431건 (서울 6구) | 국토교통부 실거래가 공개 API |
...
| 소비 성향 | 경기데이터드림 카드소비 집계 (2026-03) |
| 통근·상권 | ODsay 대중교통 · 소상공인시장진흥공단 상가정보 |
