...
**두 경로 (`llm_active`로 분기) — 코드와 1:1:**
- **LLM 경로**(키+`LLM_ENABLED=true`): 자유입력 해석을 Claude가 한다. 단 출력은 **정해진 4축(commute/consumption/budget/preference)으로만 제약** — 새 축·항목·숫자(배수 0.3~2.0 범위 밖) 창작 시 **스키마 검증 실패 → 즉시 키워드 폴백**(`clarify._llm_interpret`).
- **키워드 폴백**(LLM 비활성/실패): `_NOTE_MAP` 규칙 매핑. 오프라인·테스트에서 동일 플로우 완주.
- **모순 감지는 두 경로 모두 결정론**(`_household_conflict`). LLM은 감지된 모순의 **되묻기 문구만** 자연스럽게 다듬는다(감지 로직엔 LLM 안 들어감).
- **닫힌 루프**: 명확화 질문 → 사용자 확정 → 결정론 계산. 매 실행 같은 입력에 같은 답 → 재현·감사 가능(자율 계획과 다름, §4-5).

> ⚠️ 경계(중요): LLM은 여기서 **해석·되묻기·제안 가중치**까지 만든다. **실제 동네 랭킹의 가중치는 persona/scoring이 결정론(keyword)으로** 계산한다(persona 결정론 원칙). 둘 다 같은 note에서 출발.
...
- `scoring_ctx()` → 동네 스코어 입력 **단일 소스**. 자유입력 보정 가중치(**keyword 결정론**, `clarify.note_weights`)를 실어 순위에 반영.
...
- `★` = 명확화·조합 노드. Langfuse 한 trace에 nested → 발표 시 판단 경로 라이브 노출.
- **실제 LLM 호출 지점**: `clarify`(자유입력 해석, 축 제약+폴백) · `narrate`(통역) · 상품 matcher. `compare`·`route`·`persona`·`scoring`은 **전부 결정론**.
...
- **개인 실소비**(마이데이터·KB카드) → PoC는 연령 세그먼트 근사(traits는 카드통계 도출이지 '이 사람' 실측 아님). 실서비스 해자.
- **clarify LLM 해석** → **구현됨**(`llm_active` 시 Claude, 아니면 키워드 폴백). 단 랭킹 가중치는 결정론 keyword 경로를 쓴다(persona 결정론). LLM 조정을 랭킹까지 흘리는 건 다음 단계(regions가 clarify 결과를 받도록 배선 필요).
