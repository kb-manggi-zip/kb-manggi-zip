...
### A. 경계 — 어디가 에이전트고 어디가 규칙인가

| 레이어 | 방식 | 왜 이렇게 |
|---|---|---|
| 계산 (`compare.py`) | 🔒 **규칙기반 결정론** | 금융 정확도·감사·환각 0 |
| 데이터 (실거래·규정) | 🔒 **규칙기반** (파라미터 SQL·YAML) | 정확·안전·재현 (LLM이 SQL 짜면 위험) |
| **이해** (`intake`) | 🤖 **LLM** | 사용자 상황을 읽고 *어떤 관점이 중요한지* 판단 |
| **통역** (`narrate`) | 🤖 **LLM + persona frames** | 같은 숫자를 *그 사람 언어로* |
| 오케스트레이션 | LangGraph + Langfuse | 이해→계산→통역을 추적가능한 한 흐름으로 |

> 정직한 위치: 지금은 **선형 에이전틱 워크플로우**(자율 멀티에이전트 스웜 아님). 금융앱은 **예측가능성 > 자율성**이라 이게 옳은 선택 — 돈 계산을 LLM이 자율로 하게 두지 않는다.
