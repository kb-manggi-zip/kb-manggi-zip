...
> **더 강한 에이전틱을 원하면(선택 확장)**: `supervisor` 라우팅 노드 — LLM이 "이 사용자에겐 매매가 비현실적이니 2갈래만", "갱신 선택 시 지역·하루 스킵"처럼 **경로 자체를 판단**하게 만들면 동적 제어흐름(=진짜 자율 에이전트)이 된다. 현재는 이 분기를 **화면 흐름이 결정론으로** 담당한다. 안전과 데모 명료성을 위해 지금은 선형을 택했다.
