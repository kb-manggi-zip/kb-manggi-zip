...
- 실거래 열: 서버는 외부 API를 호출하지 않고 **SQLite**에서 읽는다. 읽기 DB 선택: `TRADES_DB` env > `data/trades.db`(단, 통근 데이터가 비면 데모로 폴백) > `data/trades.demo.db`. 기동 시 어느 DB를 읽는지 로그 1줄(`trades_store.resolve_db_path`, 가드는 `test_db_guard.py`).
...
| POST | `/api/analyze` | LangGraph 6노드 통합(intake→clarify→compare→route→persona→narrate) | ✅* | rules·DB |
| POST | `/api/clarify` | 자유입력→4축 배수 해석 | ✅* | - |
| POST | `/api/validate-profile` | 최종 프로필 모순 검증(HITL) | ✅* | - |
| POST | `/api/persona` | 페르소나 프로필 조합 | ❌ | 세그먼트·mydata |
| GET | `/api/regions` | 동네 후보 3곳(개인 가중 순위) | ❌ | trades·transit·facts DB |
| POST | `/api/report` | 만기 결정 리포트 조합 | ✅* | 위 산출물 조합 |
| POST | `/api/hitl` | HITL 확정 이벤트 기록 | ❌ | (관측) |
| POST | `/api/simulate` | 하루 씬 | ❌ | scenes |
| POST | `/api/products` | 갈래별 KB 상품 + 사유 | ✅* | 상품문서(RAG) |
| POST | `/api/briefing` · `/briefing/stream` | 화면 통역 문장(+SSE) | ✅* | templates |
| POST | `/api/draft-notice` | 갱신 통보 문자 초안 | ✅* | template |
| POST | `/api/reservation` | 상담 예약 저장 | ❌ | SQLite |

`*` = LLM 지점(키 없으면 템플릿/규칙 폴백). 노드별 판단 주체·왜 LLM인지는 [에이전트_설계.md](에이전트_설계.md).
...
## POST /api/analyze
LangGraph 6노드를 한 번에 실행(intake→clarify→compare→route→persona→narrate). 한 요청=한 관측 trace. 요청은 `CompareRequest`와 동일, 응답 `AnalyzeResponse`(비교표+페르소나+발품 요약). 세부 노드 역할은 [에이전트_설계.md](에이전트_설계.md).

## POST /api/clarify
자유입력(note)을 4개 축 배수로 해석. LLM 활성 시 Claude, 아니면 `_NOTE_MAP` 키워드 폴백. 배수는 0.3~2.0으로 제약. **응답** `ClarifyResult`(`weightAdjust`, `noteSignals`, `conflicts`, `held`).
...
## POST /api/hitl
HITL 확정 이벤트 기록(관측). `{ choice, signals, note }` → 세션 span에 `hitl_persona_confirm`.

## POST /api/simulate
선택 갈래×동네의 "하루" 씬. `scenes.yaml` 콘텐츠(코드-콘텐츠 분리).
**요청** `{ "branch": "매매", "regionId": "mapo" }` → **응답** `{ "scenes": Scene[5], "monthlyCost": int }`

## POST /api/products
갈래별 KB "대출+보장" 패키지. 자격은 결정론 필터, **추천 사유만 LLM**(상품 문서 `data/kb_products/*.md` 근거, 키 없으면 문서 원문 사유). 권유 표현 금지(verify 차단).
**요청** `{ "branch": "매매", "comparison": CompareResponse?, "contract": ContractInfo? }`
