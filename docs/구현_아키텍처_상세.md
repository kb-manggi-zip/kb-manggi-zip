...
# 출처: 2024 주거실태조사 통계보고서 <표 10> '이사 이유'(전국·복수응답). 4축 전부 원문값(W1 확정).
#  commute 직주근접 30.6 / consumption 교통·편의·녹지 복합 25.5 / preference 가족근접8.1+교육3.0=11.1 / budget 집값부담 8.3
#  ※ W1 정정: 과거 25.5를 통근에 합산(56.1)한 오독을 바로잡아 consumption으로 재매핑. → 정규화 0.405/0.338/0.147/0.110
```
...
**P1 실제 조정 (조정 전 → 후)** — ⚠️ W1으로 base 가중치가 바뀌어 아래 절대값은 재계산됨(1인 base: commute .469 · consumption .301 · budget .098 · preference .131):
```
before(가구 통계만): commute 0.469 · consumption 0.301 · budget 0.098 · preference 0.131
after ("재택근무예요"): 통근(×0.5)·생활(×1.3) 보정+재정규화 → 통근↓·생활소비↑ (재택 확정 시)
→ 통근 기본값이 여전히 높아 순위 1위는 유지되나 값 이동은 뚜렷 — W1 이후 생활·소비 축이
   더 커져(1인 30%) 카페 신호까지 더하면 생활·소비가 1위로 역전되기 쉬움.
```
...
{ "segment": "1인 청년 임차 가구",
  ...
  "workplace": "판교(IT)",
  "weights": {"commute":0.335,"consumption":0.287,"budget":0.166,"preference":0.212},
