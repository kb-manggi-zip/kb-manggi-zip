...
```python
SURVEY_RATIOS = {"commute":0.561, "consumption":0.24, "budget":0.18, "preference":0.23}
# 출처: 국토부 2024 주거실태조사 '이사 사유' — 직주근접30.6 + 교통편리25.5 = 56.1%(확정)
```
...
**P1 실제 조정 (조정 전 → 후):**
```
before(가구 통계만): commute 0.528 · consumption 0.174 · budget 0.131 · preference 0.167
after ("재택근무예요"): commute 0.335 · consumption 0.287 · budget 0.166 · preference 0.212
→ 통근이 0.528→0.335로 크게 낮아지고 생활·소비가 0.174→0.287로 상승. (1인 통근 기본값이 워낙
   높아 순위 1위는 유지되지만, 값의 이동은 뚜렷 — 카페까지 더하면 생활·소비가 1위로 역전)
```
