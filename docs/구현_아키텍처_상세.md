...
> E2E 문서(`서비스_E2E_아키텍처.md`)의 각 단계를 **실제 파일·함수·실행값**으로 세분화한다.
...
> KB 만기상담소 = **하나의 에이전틱 흐름**을 이루는 6개 기능. 이름표는 붙이되 설명·수치는 전부 실제 코드/실측값.
...
| ② | **3갈래 채점 엔진** | `compute_compare`+`rules/*.yaml` | 갱신·이사·매매를 법령 결정론으로 채점(HF 계산기 오차 0.6원) |
...
- **① 페르소나** — (a) 사용자가 "재택이라 통근 덜 중요해요"처럼 표준 폼으로 안 잡히는 말을 할 때. (b) P1: "재택근무예요" → 통근 가중치 0.528→0.335(§A-2). (c) 프로필 traits는 경기 카드소비(2026-03) 세그먼트 도출 — 개인 실측(마이데이터)은 실서비스 해자로 정직 구분.
...
### ③ 실거래 — `trades.db::trades` (64,496행 · 국토부 실거래 실데이터 스냅샷, O1)
```
...
### ④ 상권·통근 — `trades.db::region_facts`(27행) / `region_transit`(45행)
```
...
```python
SURVEY_RATIOS = {"commute":0.306, "consumption":0.255, "budget":0.18, "preference":0.23}
# 출처: 국토부 2024 주거실태조사 '이사 사유' — 직주근접30.6(확정, 순수 commute).
# "교통편리·편의문화시설·공원녹지"25.5%는 통근시간이 아니라 상권·편의시설 매칭(consumption)에
# 대응돼 2026-07-30 재배선(예전엔 전부 commute로 오매핑, 56.1%였음).
```
...
```
noteSignals: ["재택·동네생활 중시 → 통근 가중치 절반·생활편의↑"]   # _NOTE_MAP 규칙
conflicts  : []
```
...
[3] 본인 진술 우선    : note 보정 — "재택" → commute ×0.5, "카페" → consumption ×1.3
...
| note 키워드 | 축 | 배수 |
|---|---|---|
| 재택/집에서/동네에서 | commute / consumption | ×0.5 / ×1.3 |
| 자차/운전 | commute | ×0.7 |
...
**P1 실제 조정 (조정 전 → 후):**
```
before(가구 통계만): commute 0.374 · consumption 0.240 · budget 0.169 · preference 0.217
after ("재택근무예요"): commute 0.211 · consumption 0.353 · budget 0.191 · preference 0.245
→ 통근이 0.374→0.211로 낮아지고 생활·소비가 0.240→0.353으로 상승, 순위 1위가 통근→생활·소비로 역전.
```
...
{ "segment": "1인 청년 임차 가구",
  ...
  "workplace": "판교(IT)",
  "weights": {"commute":0.211,"consumption":0.353,"budget":0.191,"preference":0.245},
...
```
- 실측 예(이사 후보): 응암동 전세중위 5.2억 ÷ 매매중위 7.95억 = **65% [여유]** (매매 536건 기준) · 하월곡동 56%(252건).
...
### 검증 장치
HF 공적 계산기 앵커(3.2억·2.85%·30년 → 월 132만, 오차 0) + `engine ↔ tools` 동치 테스트(`gen_fixtures.mjs`) → 이 계산이 재현·감사됨.
...
```
P1(매매 예산 6.28억, weights commute 0.335·consumption 0.287…) 실제 top3:

| 동네 | 중위가 | score | breakdown(commute/budget/consumption/preference) |
|---|--:|--:|---|
| 성북동 | 5.7억 | 0.604 | 0.5 / 0.999 / 0.5 / 0.6 |
| 당인동 | 5.6억 | 0.604 | 0.5 / 0.998 / 0.5 / 0.6 |
| 신내동 | 5.9억 | 0.598 | 0.5 / 0.961 / 0.5 / 0.6 |
