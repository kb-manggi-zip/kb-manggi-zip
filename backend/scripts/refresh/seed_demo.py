...
SALE = [("11440", "합정동", 530_000_000), ("11380", "녹번동", 490_000_000), ("11320", "창동", 430_000_000)]
# 전세보증금 중심
JEONSE = [("11290", "길음동", 290_000_000), ("11350", "상계동", 260_000_000), ("11260", "면목동", 230_000_000)]
# (보증금 중심, 월세 중심) — 월세
MONTHLY = [("11440", "망원동", 72_000_000, 1_950_000), ("11290", "보문동", 55_000_000, 1_700_000),
...
def _rows(rng, sigungu, umd, tt, price_c, deal_ym, monthly_c=0):
    ...
    for _ in range(PER_DONG):
        ...
        out.append({"sigungu_code": sigungu, "umd_name": umd, "trade_type": tt,
                    "price": price, "monthly": mon, "area_m2": rng.choice(AREAS), "deal_ym": deal_ym})
    return out
...
def run() -> None:
    ...
    for sig, umd, dc, mc in MONTHLY:
        for ym in months:
            trades_store.replace_batch(conn, sig, ym, "monthly",
                                       _rows(rng, sig, umd, "monthly", dc, ym, monthly_c=mc))
